...
# do the transposing properly - in a tidyverse way so that it can be downloaded
```
...
my_XY_dataset <- reactive({
  ...
  my_dataset |>
    ...
    tidyr::pivot_wider(names_prefix = "Y_", values_from=input$XY_Yval, names_from = input$XY_Ygroup)
})  
```
...
output$XY_output <- renderUI({
  #browser()
  create_table_UI("XY_table", isTruthy(dataset()))
})
...
observeEvent(dataset(), {
  ...
  if(! is.data.frame(my_transposed_dataset()))  shinyjs::hide("download_transposed")
  ...
  if(length(column_names) > 2) {
    ...
    #updateSelectInput(inputId = "XY_id", label = "Choose ID column", choices = c("Add ID" = "add_ID", column_names))
...
output$transposed_output <- renderUI({
  create_table_UI("transposed_table", isTruthy(dataset()), msg = "This format will only download properly if the dataset has (unique) row and column names.")
})
